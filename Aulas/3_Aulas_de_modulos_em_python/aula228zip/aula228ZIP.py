...
CAMINHO_ZIP_DIR = CAMINHO_RAIZ / 'aula_228_diretorio_zip'
CAMINHO_COMPACTADO = CAMINHO_RAIZ / 'aula228_compactado.zip'
CAMINHO_DESCOMPACTADO = CAMINHO_RAIZ / 'aula228_descompactado'
...
with ZipFile(CAMINHO_COMPACTADO, 'w') as zip:
    for root, dirs, files in os.walk(CAMINHO_ZIP_DIR):
        for file in files:
            # print(file)
            # zip.write(os.path.join(root, file)) #cria a estrutura de pastas do caminho 
            zip.write(os.path.join(root, file), file) #cria só os arquivos no zip

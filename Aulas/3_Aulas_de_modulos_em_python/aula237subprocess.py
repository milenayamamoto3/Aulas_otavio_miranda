...
# Retorno: #quando da o 'print'
# stdout, stderr, returncode(se printar'0' significa"sem erros")e args
# Importante: a codificação de caracteres do Windows pode ser
# diferente. Tente usar cp1252, cp852, cp850 (ou outros). Linux e
# mac, use utf_8.
# Comando de exemplo:
# no windowns vem por padrão 4
# Windows: ping 127.0.0.1
# nesses outros so precisa por "-c n" sendo o n o nº de vezesn
# Linux/Mac: ping 127.0.0.1 -c 4
import subprocess
import sys

# sys.platform = linux, linux2, darwin, win32
# print(sys.platform)  # printa o seu so

encoding = "utf_8"
cmd = ["ls -lah /"]  # comando listando
# os args juntos qnd usar o "shell=True"
...
proc = subprocess.run(
    cmd, capture_output=True, text=True, encoding=encoding, shell=True
)

print()

# print(proc.args)
# print(proc.stderr) # b significa byte no terminal
# print(proc.stdout.decode("cp850"))  # decodificar para utf 8 no windowns
print(proc.stdout)
